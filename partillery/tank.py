...
class Tank(pygame.sprite.DirtySprite):
    def __init__(self, background, playsurf_rect, col, proj_x, center, slope_radians, turret_angle):
        super(Tank, self).__init__()
        self.surf_org = utils.load_image_resource('tank_' + col + '.png').convert_alpha()
        ...
        self.eraser = get_eraser(background, self.rect)  # save new eraser
        self.turret_nose = None
        self.crosshair = CrossHair(background, playsurf_rect, center, turret_angle, self.w_orig * 4)
        background.blit(self.surf, self.rect)  # initial tank draw
        ...
        self.combo_eraser = get_eraser(background, self.combo_erase_loc)
        self.update_turret(background, turret_angle, True)  # initial turret draw
...
def get_eraser(surface, rect):
    area = surface.subsurface(rect).copy()
    return area
