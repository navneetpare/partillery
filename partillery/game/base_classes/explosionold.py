...
class ExplosionOld:
    def __init__(self, screen, playsurfbk, terrain_mask, center, radius, ammo_type):
        ...
        exp_speed = 0.1  # pixels per 1000 ms
        ...
        while exp_speed * t < radius:  # milliseconds
            for i in range(1, int(exp_speed * t)):
                ...
                #pygame.draw.circle(screen, (255, 150, 0, int(255 * t / 2500)), center, i, 1)
                #pygame.draw.circle(screen, (255, 150, 0, int(255 * (1 + math.cos(i / 3)) / 2)), center, i, 1)
                pygame.draw.circle(screen, (255, 150, 0, int(t % i)), center, i, 1)
...
class BaseExplosion:
    def __init__(self, screen, sky: pygame.Surface, terrain: Terrain, pos: tuple, radius: int, lifespan: int):
        ...
        exp_speed = self.radius / self.lifespan
        t = 0
        t0 = pygame.time.get_ticks()

        # Getting a circular alpha mask for the explosion
        exp_rect = pygame.Rect(0, 0, self.radius * 2, self.radius * 2)  # create a rect the size of the exp
        exp_rect.center = self.pos  # position the rect at the explosion location
        exp_rect = exp_rect.clip(self.sky.get_rect())  # clip the rect to sky frame for edge cases
        exp_bg = self.sky.subsurface(exp_rect).copy()  # get a backup of the sky at the exp location
        ...
        x, y = ogrid[:exp_rect.width, :exp_rect.height]
        # find the center of the backup rect - remember this is not exp loc, but the backup which corners at (0,0)
        exp_bg_center = (self.radius, self.radius)
        # No 'for' loops. Function uses broadcasting for the formula, so 'd' will be an array too - the power of Numpy!!
        d = sqrt((x - exp_bg_center[0]) ** 2 + (y - exp_bg_center[1]) ** 2)
        exp_mask = d < self.radius  # so is the mask
        exp_bg_array[~exp_mask] = 0  # this directly alters the pixel alphas due to the fast pygame method

        del exp_bg_array  # delete the pixel alpha reference array to unlock the bg surface

        # -------- Create mask to delete from terrain
        cut_surf = pygame.Surface((self.diameter, self.diameter))
        pygame.draw.circle(cut_surf, (255, 255, 255, 255), (self.radius, self.radius), self.radius)
        terr_cut_mask = pygame.mask.from_threshold(cut_surf, (255, 255, 255, 255))
        terr_cut_mask.invert()
        exp_speed = 0.1  # pixels per 1000 ms
        ...
        self.screen.blit(exp_bg, exp_rect)

        self.terrain.mask.erase(terr_cut_mask, (self.pos[0] - self.radius, self.pos[1] - self.radius))
        pygame.display.update(exp_rect)

        # this creates a fuzzy fireball that clarifies with time
        # pygame.draw.circle(self.screen, (255, 150, 0, int(255 * t / 2500)), self.pos, i, 1)

        # this creates a striped growing explosion - cosine
        # pygame.draw.circle(self.screen, (255, 150, 0, int(255 * (1 + math.cos(i/3)) / 2)), self.pos, i, 1)

        # weird buzzy fireball
        # pygame.draw.circle(self.screen, (255, 150, 0, int(t % i)), self.pos, i, 1)
        # return exp_rect